      ...
      <CardHeader className="pb-3">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          ...
          <CardTitle className="text-lg leading-tight">
            {order.items[0]?.serviceName || "Order Details"}
          </CardTitle>
          <div
            className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full border ${getStatusChipStyles(
              order.status
            )}`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          ...
          <div className="flex items-center">
            ...
            {order.status === "Delivered" && (
              <CircleCheck size={14} className="mr-2 text-green-600" />
            )}
            {order.status === "Cancelled" && (
              <CircleAlert size={14} className="mr-2 text-red-600" />
            )}
            <div>
              ...
              <span className="font-medium ml-1 text-slate-800">
                {order.status === "Delivered" && order.actualDeliveryDate
                  ...
                  : order.status !== "Cancelled" && order.estimatedDeliveryDate
